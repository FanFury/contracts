...
pub fn instantiate(
    ...
    let mut cfg = Config {
        ...
        native_investment_reward_wallet: deps.api.addr_validate(
            &msg.native_investment_reward_wallet,
        )?,

        authorized_liquidity_provider: deps.api.addr_validate(
            &msg.authorized_liquidity_provider.as_str(),
        )?,
        native_investment_receive_wallet: deps.api.addr_validate(
            &msg.native_investment_receive_wallet,
        )?,
        swap_opening_date: Timestamp::from_nanos(msg.swap_opening_date.u64()),
        pool_pair_address: String::default(),
        pair_lp_tokens_holder: deps.api.addr_validate(
            &msg.pair_lp_tokens_holder,
        )?,
